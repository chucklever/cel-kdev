...
Queueing a patch, on a local daemon or a private remote
instance, needs mbox headers the local export tools omit by
default, and most of the ways it fails report success: a
message the parser drops, a series that fragments into
`Incomplete` patchsets, and a bare `submit` that reviews the
daemon's own tree all exit 0.  Read
[references/submitting.md](references/submitting.md) before
the first `sashiko-cli submit`.  It carries the header
recipes for a single patch and for a series, which `--type`
each input form selects, the write-access gate and the two
ways through it, the poll that confirms a submit landed, and
the `received_parts` check that catches a fragmented series.
