...
If `sudo` is unavailable or the user requests
unprivileged profiling, there are two alternatives.
Check the current level with
`sysctl kernel.perf_event_paranoid` -- values `>=2`
restrict monitoring to user-space per-process events
...
- **Lost samples**: If perf warns about lost samples
  or `perf report --header-only` shows
  `PERF_RECORD_LOST`, overhead percentages are
  unreliable -- the highest-rate functions are the
  most undercounted. See "Ring buffer sizing" under
  Recording for diagnosis and the `-m` fix.
